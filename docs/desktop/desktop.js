...
var Desktop = function() {
	...
	window.document.addEventListener("DOMContentLoaded",function(event) {
		window.console.log("" + App.NAME + " - Desktop - Dom ready :: build: " + "2020-05-08 11:43:17");
		_gthis.init();
	});
};
...
Desktop.prototype = {
	...
	,startTime: function() {
		...
		var _h = this.checkTime(h);
		var _m = this.checkTime(m);
		var _s = this.checkTime(s);
		this.timeEl.innerHTML = _h + "<span class='dotdot'>:</span>" + _m + "<span class='dotdot'>:</span>" + _s;
		var t = window.setTimeout($bind(this,this.startTime),500);
	}
	,randomBg: function() {
		var url = this.bgArray[Math.floor(Math.random() * this.bgArray.length)];
		this.containerDesktop.style.backgroundImage = "url(" + url + ")";
	}
	,randomQuote: function() {
		var randomQuote = this.quoteArray[Math.floor(Math.random() * this.quoteArray.length)];
