...
class HtmlAudioWrapper {
	...
	init(id) {
		...
		if(wrapper.getElementsByTagName("audio")[0] == null) {
			console.log("src/HtmlAudioWrapper.hx:107:","no <audio> tag in element $id");
			return;
		}
		let audio = js_Boot.__cast(wrapper.getElementsByTagName("audio")[0] , HTMLAudioElement);
		...
		scrubber.onclick = function(e) {
			...
			if(isNaN(f)) {
				console.log("src/HtmlAudioWrapper.hx:131:","start loading data?");
				return;
			}
		...
		audio.onerror = function(e) {
			console.log("src/HtmlAudioWrapper.hx:188:","## debug");
	...
	static main() {
		let app = new HtmlAudioWrapper();
	}
}
HtmlAudioWrapper.__name__ = true;
Object.assign(HtmlAudioWrapper.prototype, {
	__class__: HtmlAudioWrapper
});
class HxOverrides {
	static cca(s,index) {
